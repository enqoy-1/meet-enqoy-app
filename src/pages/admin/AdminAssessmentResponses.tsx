...
const AdminAssessmentResponses = () => {
  ...
  const filteredResponses = responses.filter((response) => {
    ...
    return (
      ...
      response.profiles?.phone?.includes(searchLower)
    );
  });
  ...
  const exportToCSV = () => {
    const headers = [
      ...
      "Has Children",
      "Submission Date",
      "Phone (from assessment)",
      "Which city would you like to attend Enqoy from?",
      "Specified City (if outside Addis)",
    ...
    const csvData = filteredResponses.map((response) => {
      ...
      return [
        ...
        response.profiles?.age || "",
        response.profiles?.gender || "",
        response.profiles?.city || "",
        response.profiles?.relationshipStatus || "",
        response.profiles?.hasChildren !== undefined ? (response.profiles.hasChildren ? "Yes" : "No") : "",
        new Date(response.created_at).toLocaleDateString(),
        answers.phone || "",
        getFullAnswerText("city", answers.city),
        answers.specifiedCity || "",
        getFullAnswerText("preferredTime", answers.preferredTime),
        getFullAnswerText("dinnerVibe", answers.dinnerVibe),
        getFullAnswerText("talkTopic", answers.talkTopic),
        getFullAnswerText("groupDynamic", answers.groupDynamic),
        getFullAnswerText("humorType", answers.humorType),
        getFullAnswerText("wardrobeStyle", answers.wardrobeStyle),
        answers.introvertScale ? `${answers.introvertScale}/5` : "",
        answers.aloneTimeScale ? `${answers.aloneTimeScale}/5` : "",
        answers.familyScale ? `${answers.familyScale}/5` : "",
        answers.spiritualityScale ? `${answers.spiritualityScale}/5` : "",
        answers.humorScale ? `${answers.humorScale}/5` : "",
        getFullAnswerText("meetingPriority", answers.meetingPriority),
        getFullAnswerText("dietaryPreferences", answers.dietaryPreferences),
        answers.customDietary || "",
        getFullAnswerText("restaurantFrequency", answers.restaurantFrequency),
        getFullAnswerText("spending", answers.spending),
        answers.country || "",
